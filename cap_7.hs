...
-- This is a bit different respect takeWhile because we skip the elements
-- with the predicate false and consider all the elements with p=true
own_takeAll :: (a -> Bool) -> [a] -> [a]
own_takeAll p [] = []
own_takeAll p (x:xs)
    | p x = x : own_takeAll p xs
    | otherwise = own_takeAll p xs
...
main = do
    ...
    print(own_takeAll isAlpha "abc def")    
